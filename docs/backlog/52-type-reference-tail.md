...
explicit call-site type arguments) belong to the same family. Qualified names `A.B`
(TS2503) are covered by `43`.
...
Validate every type-reference against the target symbol: value-only symbol → TK2749
...
`src/check/checker/annotations.rs` (type-reference lowering), `src/check/checker/calls.rs`

...
| 1c | Depth-tainted cache entries (rustc; same class as our b55) | **NOTE → invariants sentence** | Vacuous today — no depth/budget exists in `src/relate/` (TK2589 is evaluator-only, already b55-guarded). Becomes mandatory the day a relation depth limit is added; recorded as one sentence in `invariants.md` §1, not a backlog item. |

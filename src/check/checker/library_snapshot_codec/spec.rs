//! RED contract for the semantic-snapshot feasibility prototype.
//!
//! This is not the production `FrozenLibraryBase` design. The feasibility prototype may consume one decoded
...
//!
//! The feasibility wire header is fixed by the constants below. Directory entries are
...
//!
//! `snapshot_fast_clean_probe_once` is an ignored release-process primitive. The external
//! coordinator runs each strategy in fresh processes, records validation, decode/base construction,
//! user check, wall time, artifact bytes and peak RSS, and compares semantic
...
use super::profile::load_strict_profile;
use super::{
...
fn decoded_user_route_has_no_source_compiler_dependency() {
    ...
    let compiler_source = include_str!("mod.rs");
...
#[test]
#[ignore = "release-process snapshot evidence probe; run through the external coordinator"]
...
#[test]
#[ignore = "snapshot 1/2/32 shape probe; not a shared-base or parallelism claim"]

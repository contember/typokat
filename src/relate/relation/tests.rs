use super::*;
use crate::class_semantics::{ClassConstructionState, Exhaustion, PublishedClasses};
...
use crate::types::repr::{
    ...
    ObjectType, ParameterType, PropertyType, TemplateType, TupleRestType, TupleType, Visibility,
};
...
fn dormant_relation_roots(interner: &mut Interner, dormant: TypeId) -> Vec<(&'static str, TypeId)> {
    ...
    roots.extend([
        ...
        (
            ...
            interner.intern_instantiation(wk.number, vec![(TypeParamId(81_003), dormant)]),
        ),
        (
            "class-instance argument",
            interner.intern_class_instance(ClassId(81_004), vec![dormant]),
        ),
    ]);
    ...
    let parameter_id = TypeParamId(81_005);
...
#[test]
fn legacy_relation_preflight_covers_every_semantic_child_role_without_writes() {
    reset_relation_guard_measure();
    let mut interner = Interner::with_intrinsics();
    let wk = interner.well_known();
    let dormant = interner.intern_class_instance(ClassId(81_000), vec![wk.string]);
    let roots = dormant_relation_roots(&mut interner, dormant);
    ...
    for reversed in [false, true] {
        ...
        for (name, root) in order {
            ...
            let result = catch_unwind(AssertUnwindSafe(|| relater.is_assignable(*root, wk.number)));
            assert!(result.is_err(), "missing preflight child role: {name}");
            assert!(
                relater.cache.is_empty(),
                "cache write before rejecting {name}"
            );
            assert!(
                relater.stack.is_empty(),
                "cycle push before rejecting {name}"
            );
        }
    }
    assert_eq!(
        relation_guard_measure().tripwires,
        u64::try_from(roots.len() * 2).unwrap()
    );
}

#[test]
fn legacy_relation_empty_store_gate_skips_the_policy_walk() {
    reset_relation_guard_measure();
    ...
    assert!(matches!(
        relater.is_assignable(wk.number, wk.string),
        Relation::No(_)
    ));
...
#[test]
fn defensive_relation_frame_rejects_before_identity_cache_or_cycle_state() {
    reset_relation_guard_measure();
    let mut interner = Interner::with_intrinsics();
    let wk = interner.well_known();
    let application = interner.intern_class_instance(ClassId(81_006), Vec::new());
    let mut relater = Relater::new(interner.store(), wk);
    let _ = relater.is_assignable(wk.number, wk.string);
    let cache_len = relater.cache.len();
    ...
    assert_eq!(relater.cache.len(), cache_len);
    assert!(relater.stack.is_empty());
    assert!(assumed.is_empty());
}

#[test]
fn legacy_relation_rejects_dormant_identity_before_fast_path_and_cache() {
    reset_relation_guard_measure();
    let mut interner = Interner::with_intrinsics();
    let wk = interner.well_known();
    let application = interner.intern_class_instance(ClassId(30), vec![wk.number]);
    let mut relater = Relater::new(interner.store(), wk);

    let result = catch_unwind(AssertUnwindSafe(|| {
        relater.is_assignable(application, application)
    }));

    assert!(result.is_err());
    assert!(relater.cache.is_empty());
    assert!(relater.stack.is_empty());
    assert_eq!(
        relation_guard_measure(),
        RelationGuardMeasure {
            tripwires: 1,
            root_walks: 1,
        }
    );
...
#[test]
fn legacy_relation_preflights_nested_dormant_nodes_before_cache_writes() {
    reset_relation_guard_measure();
    let mut interner = Interner::with_intrinsics();
    let wk = interner.well_known();
    ...
    let result = catch_unwind(AssertUnwindSafe(|| relater.is_assignable(nested, target)));

    assert!(result.is_err());
    assert!(relater.cache.is_empty());
    assert!(relater.stack.is_empty());
    assert_eq!(relation_guard_measure().tripwires, 1);
}

#[test]
fn typed_relation_seam_rejects_every_prepublication_state_without_cache() {
    reset_relation_guard_measure();
    let mut interner = Interner::with_intrinsics();
    let wk = interner.well_known();
    let class = ClassId(31);
    ...
    for state in [
        ...
        let forged = PublishedClasses::forged(class, state);
        assert!(matches!(
            relate_unevaluated_deferred(&forged, interner.store(), nested, nested),
            Some(RelationOutcome::Exhausted(
                Exhaustion::ClassNotPublished {
                    class: rejected,
                    state: rejected_state,
                }
            )) if rejected == class && rejected_state == state
        ));
    }
    let poisoned = PublishedClasses::forged(class, ClassConstructionState::Poisoned);
    assert!(matches!(
        relate_unevaluated_deferred(&poisoned, interner.store(), nested, nested),
        Some(RelationOutcome::Exhausted(
            Exhaustion::ClassHeritagePoison { class: rejected }
        )) if rejected == class
    ));
    assert_eq!(relation_guard_measure().tripwires, 3);
}

#[test]
fn typed_relation_publication_walks_class_arguments_and_both_deferred_children() {
    let mut interner = Interner::with_intrinsics();
    let wk = interner.well_known();
    let outer_class = ClassId(31_001);
    let inner_class = ClassId(31_002);
    ...
    let deferred_object = interner.intern_deferred_indexed_access(inner, wk.string);
    let deferred_index = interner.intern_deferred_indexed_access(wk.number, inner);
    let published = PublishedClasses::forged_states([
        (outer_class, ClassConstructionState::Published),
        (inner_class, ClassConstructionState::Pending),
    ]);

    for root in [outer, deferred_object, deferred_index] {
        assert!(matches!(
            relate_unevaluated_deferred(&published, interner.store(), root, wk.number),
            Some(RelationOutcome::Exhausted(
                Exhaustion::ClassNotPublished {
                    class: rejected,
                    state: ClassConstructionState::Pending,
                }
            )) if rejected == inner_class
        ));
    }
}

#[test]
fn unevaluated_deferred_relation_is_identical_only() {
    let mut interner = Interner::with_intrinsics();
    let wk = interner.well_known();
    let left = interner.intern_deferred_indexed_access(wk.number, wk.string);
    let right = interner.intern_deferred_indexed_access(wk.string, wk.number);
    let published = PublishedClasses::empty();

    assert!(matches!(
        relate_unevaluated_deferred(&published, interner.store(), left, left),
        Some(RelationOutcome::Yes)
    ));
    assert!(matches!(
        relate_unevaluated_deferred(&published, interner.store(), left, right),
        Some(RelationOutcome::No(_))
    ));
}

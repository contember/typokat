...
//! telemetry state is `Option<SubstitutionMeasure>` and starts as `None` on every
//! thread. `start_substitution_measure()` installs a fresh measure and returns a
//! `SubstitutionMeasureScope` RAII guard that restores the prior state on drop.
//! `substitution_measure()` is a snapshot: `None` means disabled, while `Some`
//! means the current scope is enabled even when all counters are still zero.
//!
...
//!
//! `Substitution::new` establishes once whether that whole run belongs to the
//! current scope. Every disabled per-`apply` path must branch on that run-local
//! membership before any measurement `LocalKey::with`, telemetry exact-context
//! `Vec` build/sort, or measurement set/map mutation. Every instrumentation site
//! uses that construction-time membership rather than re-deciding enrollment per
//! visit. Behavioral counters cannot prove this zero-work property, so activation
//! review must inspect every instrumentation site.
//!
...
//! guard must carry a thread-affine marker such as `PhantomData<Rc<()>>` and be
//! structurally `!Send` so its `Drop` cannot restore another thread's TLS state.
//! This crate has no direct negative-trait assertion dependency, so activation
//! review must confirm that marker.
